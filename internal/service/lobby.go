...
func (s *LobbyService) CreateLobby(options *types.DraftOptions, blueTeamName string, redTeamName string) *LobbyCreateResponse {
	s.lobbiesMutex.Lock()
	defer s.lobbiesMutex.Unlock()

	lobby := types.NewLobby(*options, blueTeamName, redTeamName)
